...
import { mediapostReplyComment } from 'services/Replymediaservice';
...
const ReplymediaModal: React.FC<ReplyFormProps> = ({ mediareplyCommentId, isOpen, onClose,username }) => {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    if (reply.trim()) {
      try {
        const newReply = await mediapostReplyComment(mediareplyCommentId, { content: reply });
  ...
  return (
    ...
    <button
      ...
      Post a Reply            </h2>
      <hr className="border-b border-gray dark:border-white mb-4 w-full" />     
...
export default ReplymediaModal;
