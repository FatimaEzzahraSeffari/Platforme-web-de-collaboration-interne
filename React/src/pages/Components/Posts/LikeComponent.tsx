...
const LikeComponent: React.FC<Props> = ({ postId }) => {
...
//pour post 
  ...
  const fetchLikes = async () => {
    try {
      const likeData: LikeDataResponse = await likeService.getLikeData(postId);
  ...
  const handleLike = async () => {
    try {
      const response = await likeService.toggleLike(postId); // No need for userId if using authentication
